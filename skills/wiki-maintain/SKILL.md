---
name: wiki-maintain
description: 审计并安全维护受管本地 Markdown wiki 的完整性、链接、孤儿页、重复实体、引用、raw 哈希、索引漂移、陈旧或冲突主张、schema、敏感度和 agent bridge。Use when the user asks to 检查、体检、lint、doctor、cleanup、清理、去重、修复、重建索引、刷新反链、审查新鲜度，或 audit, maintain, repair, rebuild, reindex, dedupe, check integrity. 审计默认只读；只有明确 repair/fix 请求才写入。
---
...
2. 将含旧 `raw/entries/`、`wiki/_index.md`、`wiki/_backlinks.json` 的 workspace 纳入审计，不要求先迁移。
3. 从本 `SKILL.md` 所在目录定位 `../wiki-configure/scripts/wiki.py`。统一使用 `python3 <wiki.py> --workspace <workspace-root> ...`；旧版 `--vault` 仍作为等价兼容参数。不得依赖当前工作目录。
...
1. 明确范围：全 workspace、某领域、某次 ingest、某类问题或某时间窗口。默认覆盖系统契约与 wiki，不扫描无关大文件。
