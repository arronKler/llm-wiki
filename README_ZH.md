...
- `wiki-configure`：初始化工作区、配置 schema 和 policy、导出派生视图，并管理 Agent discovery。
...
```text
文件、网页、在线文档、代码仓库、沟通记录、表格、API 和数据库
        ↓ 捕获
...
“把公开 Wiki 导出成静态网站。”               → wiki-configure
```
...
可以直接让 Agent 把 Wiki 的公开部分导出为静态站点；如果采用上面推荐的 `universal` 安装方式，也可以直接运行内置 CLI：
...
生成后可以直接打开 `outputs/site/index.html`，也可以使用任意静态文件服务器托管该目录。站点包含导航、安全的 Markdown 渲染、Wikilink、反向链接、本地搜索和图谱数据，不会复制原始来源文件。

默认只导出明确标记为 `public` 的页面。只有明确授权某个本地输出时，才应通过例如 `--classification internal` 增加其他密级；`public` 始终同时包含。如果选中的页面引用了未知或未获授权的来源证据，导出会拒绝继续。生成站点不等于部署或分享站点。

内置主题包括 `default`、`editorial` 和 `minimal`。可选功能包括本地搜索、页内目录、交互图谱、代码复制按钮和首页筛选。Agent 可以自动发现当前安装版本的准确选项，也可以直接组合：
...
Theme 和 Add-on 只作用于已经授权的站点快照，绝不会扩大密级范围。可以选择保存 Workspace 默认外观，也可以只通过命令行影响本次导出。
...
- 静态导出默认只包含公开页面，不复制原始来源证据，并在明确发布前始终留在本地。
